...
func (p *Pool) Get() (*hbasethrift.THBaseServiceClient, error) {
	...
	// if active too many, wait GetConTimeOut(ms)
	waitDuration := 2 //ms
	maxWaitTimes := p.GetConTimeout / waitDuration
	for waitTimes := 1; p.MaxActive > 0 && p.active >= p.MaxActive; waitTimes++ {
		p.mu.Unlock()
		if waitTimes > maxWaitTimes {
			return nil, derrors.ErrHBasePoolExhausted
		}

		t := time.Millisecond * time.Duration(waitDuration)
		time.Sleep(t)
		p.mu.Lock()
	}
