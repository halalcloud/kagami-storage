...
@app.route('/efsops/initialization', methods = ["POST"])
#@login_required
def efsopsInitPost():
	if not request.json:
		abort(400)
...
@app.route('/efsops/initialization', methods = ["GET"])
#@login_required
def efsopsInitGet():
	try:
		...
		logger.info("efsopsInitGet() called, success, initialization: %s", resp_str)
		return resp_str
	except BaseException, e:
...
@app.route('/efsops/groups', methods = ["POST"])
#@login_required
def efsopsGroupsPost():
	if not request.json:
		abort(400)
	...
	try:
		...
		if rack not in [1, 2, 3] or copys not in [2, 3] or len(ips) % copys != 0:
			logger.error("efsopsGroupsPost() called, failed, param error:  ips_length: %d copys: %d, rack: %d", len(ips), copys, rack)
			abort(400)
		for store_ip_u in ips:
			store_ip = store_ip_u.encode('utf-8')
	...
	resp_str = json.dumps(resp)
	logger.info("efsopsGroupsPost() called, success, groups: %s", resp_str)
	return resp_str


@app.route('/efsops/groups', methods = ["GET"])
#@login_required
def efsopsGroupsGet():
	try:
		resp = {}
		...
		resp_str = json.dumps(resp)
		logger.info("efsopsGroupsGet() called, success, groups: %s", resp_str)
		return resp_str
	except BaseException, e:
		logger.warn('Exception:%s', str(e))
		abort(400)
...
@app.route('/efsops/volumes', methods = ["POST"])
#@login_required
def efsopsVolumesPost():
	if not request.json:
		abort(400)
	...
	resp_str = json.dumps(resp)
	logger.info("efsopsVolumesPost() called, success, resp: %s", resp_str)
	return resp_str
