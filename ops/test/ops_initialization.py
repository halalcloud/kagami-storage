...
# initialize efs space
def space():
	url = 'http://127.0.0.1:9000/efsops/initialization'
	# ips 机器的ip列表
	# dirs 磁盘目录
	# size 每块磁盘的空间大小
	value = {"ips":"xx.xx.xx.xx, yy.yy.yy.yy","dirs":"/data1/efsdata/,/data2/efsdata/","size":"10T"}
...
# initialize groups
def groups():
	url = 'http://127.0.0.1:9000/efsops/groups'
	# ips 要分组的ip列表
	# copys 副本数（包括本身）
...
# initialize volumes
def volumes():
	url = 'http://127.0.0.1:9000/efsops/volumes'
	# groups 生效某个group
	value = {"groups":"2"}
...
#step 1:
#space()
#调用完成后，磁盘空间会被分配；efs存储目录看到生成的volume文件。
