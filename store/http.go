...
func NewServer(s *Store, c *conf.Config) (svr *Server, err error) {
	...
	if svr.rebalanceSvr, err = net.Listen("tcp", c.RebalanceListen); err != nil {
		log.Errorf("net.Listen(%s) error(%v)", c.RebalanceListen, err)
		return
	}
	...
	go svr.startApi()
	go svr.startAdmin()
	go svr.startRebalance()
	go s.backupvolume()          //定时同步volume 信息到 local index
	go s.backup_compact_volume() //后台自动压缩volume
	if c.Pprof {
